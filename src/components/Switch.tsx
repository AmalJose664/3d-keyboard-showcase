...
export function Switch({ color, hexcolor, ...restProps }: SwitchProps) {
	...
	const audioTimout = useRef<ReturnType<typeof setTimeout>>(null)

	const allAudio = useRef(SOUND_MAP[color].map((url) => {
	...
	const handlePointerDown = (event: ThreeEvent<PointerEvent>) => {
		...
		const swithGroup = switchGroupRef.current

		gsap.to(swithGroup.rotation, {
			x: Math.PI / 2 + .1,
			duration: .05,
			ease: "power2.out"
		})
		...
		audio.current.play()
		audioTimout.current = setTimeout(() => audio.current?.pause(), (audio.current.duration / 2) * 1000)
	}
	const releaseSwitch = () => {
		...
		if (audioTimout.current) clearTimeout(audioTimout.current)
		audio.current?.play()
	}
	...
	return (
		<group {...restProps}>
			...
			<mesh onPointerDown={handlePointerDown} onPointerUp={handlePointerUp} onPointerLeave={handlePointerLeave}
				position={[0, 0.05, 0]} onPointerOver={() => { document.body.style.cursor = "pointer" }}
