...
function CameraController() {
	...
	const preferReducedMotion = typeof window !== "undefined" && window.matchMedia("(prefers-reduced-motion: reduced)").matches
...
const Scene = () => {
	...
	const [lightIntnsityScaler, setLightIntnsityScaler] = useState(0)
	...
	useGSAP(() => {
		...
		mm.add("(prefers-reduced-motion: no-preference)", () => {
			...
			gsap.to({ lightIntnsityScaler: 0 },
				{
					lightIntnsityScaler: 1, duration: 3.5, delay: .5,
					ease: "power2.inOut",
					onUpdate: function () {
						setLightIntnsityScaler(this.targets()[0].lightIntnsityScaler)
					}
				}
			)
			...
			t1.to(keyboard.position, { x: 0, y: -.5, z: .5, duration: 2 })
				.to(keyboard.rotation, { x: 1.4, y: 0, z: 0, duration: 1.8 }, "<")
				.to(keyboard.position, { x: 0.2, y: -.5, z: 1.9, duration: 2, delay: .5 })
				.to(keyboard.rotation, { x: 1.6, y: .4, z: 0, duration: 2 }, "<")
				.call(() => {
					if (typeof window !== undefined) {
					...
					scrollTimeline
						.to(keyboard.position, { x: 0, y: -.5, z: 2.2 })
						.to(keyboard.rotation, { x: Math.PI * -2 + 0.8, y: 0, z: 0 }, "<")
	...
	return (
		<group>
			...
			<group scale={scalingFactor}>
				...
				// position={[.2, -.5, 1.8]} rotation={[rotateX, rotateY, rotateZ]}
				>
					<Keyboard scale={9} ref={keyboardAnimationRef} />
				</group>

				<group ref={keycapRef}>
					<Keycap position={[0, -.4, 2.6]} rotation={[0, 2, 3]} texture={0} />
					...
					<Keycap position={[.7, .9, 1.4]} rotation={[3, 2, 0]} texture={4} />
			...
			<Environment files={["/hdr/blue-studio.hdr"]} environmentIntensity={.1 * lightIntnsityScaler} />
			{/* <ambientLight intensity={.2} /> */}
			<spotLight position={[-2, 1.5, 3]} intensity={30 * lightIntnsityScaler} castShadow
				shadow-bias={-.0002}
