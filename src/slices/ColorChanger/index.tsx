...
const ColorChanger: FC<ColorChangerProps> = ({ slice }) => {
	...
	const [bgText, setBgText] = useState(KEYCAP_TEXTURES[0].name)
	const [isAnimating, setIsAnimating] = useState(false)

	function handleTextureSelect(texture: (typeof KEYCAP_TEXTURES)[number]) {
		if (texture.id === selectedTextureId || isAnimating) {
			return
		}
		setIsAnimating(true)
		setSelectedTextureId(texture.id)
		setBgText(KEYCAP_TEXTURES.find(t => t.id === texture.id)?.name || "")
	}
	const handleAnimationComplete = useCallback(() => {
		setIsAnimating(false)
	}, [])
	return (
		<section id="keycap-changer"
		...
		>
			<svg className=" pointer-events-none absolute top-0 left-0 h-auto w-full mix-blend-overlay" viewBox="0 0 75 100">
				<text
					...
					x="50%"
					y="50%"
					className=" font-black-slanted fill-white/20 group-hover:fill-white/30 motion-safe:transition-all motion-safe:duration-700">
					{Array.from({ length: 20 }, (_, i) => (
						<tspan key={i} x={`${(i + 1) * 10}%`} dy={i === 0 ? -50 : 6}>
							{Array.from({ length: 10 }, () => bgText).join(" ")}
						</tspan>
					))}
				</text>
			...
			<Bounded className=" relative shrink-0" innerClassName="gap-6 lg:gap-8 flex flex-col lg:flex-row">
				...
				<ul className="grid grow grid-cols-2 gap-3 rounded-2xl bg-white p-4 text-black shadow-lg sm:grid-cols-3 md:grid-cols-6 lg:grid-cols-3 xl:grid-cols-6">
					{KEYCAP_TEXTURES.map((texture,) => {
						return (
							<li key={texture.id}>
								<button onClick={() => handleTextureSelect(texture)} disabled={isAnimating}
